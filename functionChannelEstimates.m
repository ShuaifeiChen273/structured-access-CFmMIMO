...
Hhat = zeros(L*N,nbrOfRealizations,K,dim);
...
for l = 1:L
    ...
    for t = 1:tau_p
        ...
        for d = 1:dim
            ...
            yp = sqrt(p)*tau_p*sum(H((l-1)*N+1:l*N,:,t==pilotIndex(:,d)),3) + sqrt(tau_p)*Np(:,:,l,t);
            
            %Compute the matrix that is inverted in the MMSE estimator
            PsiInv = (p*tau_p*sum(R(:,:,l,t==pilotIndex(:,d)),4) + eyeN);
            
            %Go through all UEs that use pilot t
            for k = find(t==pilotIndex(:,d))'
                
                %Compute the MMSE estimate
                RPsi = R(:,:,l,k) / PsiInv;
                Hhat((l-1)*N+1:l*N,:,k,d) = sqrt(p)*RPsi*yp;
                
                %Compute the spatial correlation matrix of the estimate
                if nargout>2
                    B(:,:,l,k,d) = p*tau_p*RPsi*R(:,:,l,k);
                end
